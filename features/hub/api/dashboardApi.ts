...
export const dashboardApi = createApi({
  ...
  endpoints: builder => ({
    getDashboardHome: builder.query<DashboardHomeResponse, void>({
      ...
      transformResponse: (response: NestResponse<DashboardHomeResponse>) =>
        response.data,
    }),
    getDashboardSummary: builder.query<DashboardSummary, void>({
      query: () => ({ url: '/dashboard/summary' }),
      providesTags: ['Dashboard'],
      transformResponse: (response: NestResponse<DashboardSummary>) =>
        response.data,
    }),
    getActivityHeatmap: builder.query<ActivityHeatmapData[], { weeks: number }>(
      {
        ...
        providesTags: ['Dashboard'],
        transformResponse: (response: NestResponse<ActivityHeatmapData[]>) =>
          response.data,
      }
    ),
  }),
