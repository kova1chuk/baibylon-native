...
const profileSlice = createSlice({
  ...
  reducers: {
    ...
    setLanguages: (state, action: PayloadAction<{ native: string; learning: string }>) => {
  ...
  extraReducers: (builder) => {
    builder
      .addMatcher(accountApi.endpoints.getProfile.matchFulfilled, (state, action) => {
        state.loading = false;
        if (action.payload) {
      ...
      .addMatcher(accountApi.endpoints.updateLanguages.matchFulfilled, (state) => {
        state.loading = false;
        state.success = true;
      })
      .addMatcher(accountApi.endpoints.updateLanguages.matchRejected, (state, action) => {
        state.loading = false;
        state.error = (action.error as { message?: string })?.message || "Failed to save languages";
        state.success = false;
      });
  },
});
