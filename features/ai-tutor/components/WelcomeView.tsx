...
export default function WelcomeView({ mode, onModeChange, onStartChat }: WelcomeViewProps) {
          ...
          {MODES.map(({ key, labelKey, descKey, color, Icon }) => {
            ...
            return (
              <Pressable
                ...
                style={{
                  borderWidth: 2,
                  borderColor: isActive ? color : isDark ? "#27272A" : "#E7E5E4",
                  backgroundColor: isActive ? `${color}10` : isDark ? "#111113" : "#FFFFFF",
                }}
              >
                <View className="flex-row items-center gap-3">
                  ...
                  <View className="flex-1">
                    <Text className="text-base font-semibold text-foreground">{t(labelKey)}</Text>
                    <Text className="text-sm text-muted-foreground">{t(descKey)}</Text>
                  </View>
                </View>
              </Pressable>
            );
        ...
        <View className="bg-card rounded-xl p-4 mb-6">
          ...
          <View className="flex-row gap-3">
            <View className="flex-1 items-center">
              <Text className="text-lg font-bold text-foreground">{weeklyStats.totalSessions}</Text>
              <Text className="text-xs text-muted-foreground">{t("aiTutor.welcome.sessions")}</Text>
            </View>
            <View className="flex-1 items-center">
              <Text className="text-lg font-bold text-foreground">{weeklyStats.totalMessages}</Text>
              <Text className="text-xs text-muted-foreground">{t("aiTutor.welcome.messages")}</Text>
            </View>
            <View className="flex-1 items-center">
              <Text className="text-lg font-bold text-foreground">{weeklyStats.avgAccuracy}%</Text>
