...
export default function ChatScreen({ enableSuggestions = true, onQuizResult }: ChatScreenProps) {
  ...
  const { messages, isLoading, isLoadingHistory, suggestions, isConnected, sendMessage } = useChat({
    enableSuggestions,
  });
    ...
    ({ item }: { item: ChatMessage }) => {
      ...
      return (
        <View className={`mb-3 px-4 ${isUser ? "items-end" : "items-start"}`}>
          <View
            className={`rounded-2xl px-4 py-3 max-w-[85%] ${isUser ? "bg-primary" : "bg-card"}`}
          >
            ...
            <View className="bg-card rounded-xl p-3 mt-1 max-w-[85%]">
              {item.corrections.map((c, i) => (
                <View key={i} className="flex-row items-center gap-2 mb-1">
                  <Text className="text-sm text-muted-foreground line-through">{c.original}</Text>
                  <Text className="text-sm font-medium text-foreground">→ {c.corrected}</Text>
                </View>
              ))}
            </View>
          )}
