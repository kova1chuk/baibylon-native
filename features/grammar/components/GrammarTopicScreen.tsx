...
import { GrammarExplanation, GrammarSentence, GrammarTipText } from "./GrammarRichText";
...
function ExampleCard({ example }: { example: Example }) {
  ...
  return (
    <View
      ...
      style={{
        backgroundColor: isCorrect ? "rgba(16,185,129,0.05)" : "rgba(239,68,68,0.04)",
        borderWidth: 1,
        borderColor: isCorrect ? "rgba(16,185,129,0.1)" : "rgba(239,68,68,0.08)",
        borderLeftWidth: 3,
    ...
    >
      <View
        ...
        style={{
          backgroundColor: isCorrect ? "rgba(16,185,129,0.12)" : "rgba(239,68,68,0.1)",
        }}
      >
        {isCorrect ? (
      ...
      <View className="flex-1">
        ...
        {example.explanation ? (
          <Text className="text-xs text-muted-foreground mt-0.5 italic">{example.explanation}</Text>
        ) : null}
      </View>
    </View>
  );
  ...
  return (
    <View
      ...
      style={{
        ...
        borderWidth: 1,
        borderColor: isOpen ? "rgba(129,140,248,0.2)" : isDark ? "#27272A" : "#E7E5E4",
      }}
    >
      <Pressable
      ...
      >
        ...
        <Text className="flex-1 text-base font-semibold text-foreground">{rule.title}</Text>
          ...
          {rule.pattern ? (
            <View
              ...
              style={{
                ...
                borderWidth: 1,
                borderColor: isDark ? "rgba(129,140,248,0.12)" : "rgba(129,140,248,0.08)",
              }}
            >
          ...
          >
            ...
            >
              {isUpdating ? (
                ...
                <Check size={13} color={understood ? "#10B981" : "#818CF8"} strokeWidth={2} />
              )}
              <Text
      ...
      {vocabulary.map((vocab) => (
        ...
        >
          <View className="flex-row items-start justify-between gap-2">
            <View>
              <Text className="text-lg font-semibold text-foreground">{vocab.word}</Text>
              {vocab.pronunciation ? (
          ...
          <View className="flex-row items-center gap-2 mt-2">
            ...
            >
              <Text className="text-[10px] text-muted-foreground">{vocab.part_of_speech}</Text>
            </View>
            {vocab.translation ? (
              <Text className="text-sm text-muted-foreground">{vocab.translation}</Text>
            ) : null}
          </View>
...
export default function GrammarTopicScreen() {
  ...
  const [updateRuleProgress, { isLoading: isUpdatingRule }] = useUpdateRuleProgressMutation();

  const [expandedRules, setExpandedRules] = useState<Set<string>>(new Set());
  const [activeTab, setActiveTab] = useState<"rules" | "vocab">("rules");

  useEffect(() => {
    if (content?.rules && content.rules.length > 0 && expandedRules.size === 0) {
  ...
  const progressPct = totalRules > 0 ? Math.round((understoodRules / totalRules) * 100) : 0;
    ...
    >
        ...
        <View className="p-5">
          <View className="flex-row items-center justify-between mb-3">
            {content.topic.level_code ? (
              ...
              >
                <Text className="text-xs font-bold" style={{ color: "#818CF8" }}>
          ...
          <Text className="text-2xl font-bold text-foreground mb-1">{content.topic.name}</Text>
          {content.topic.description ? (
            <Text className="text-sm text-muted-foreground mb-4">{content.topic.description}</Text>
          ) : null}
          ...
          >
            <View className="flex-row items-center gap-2">
              ...
              <View>
                <Text className="text-sm font-semibold text-foreground">{totalRules}</Text>
            ...
            <View className="flex-row items-center gap-2">
              ...
              <View>
                <Text className="text-sm font-semibold text-foreground">{understoodRules}</Text>
                <Text className="text-[10px] text-muted-foreground">Understood</Text>
              </View>
            </View>
          </View>
        </View>
      ...
      >
        <Pressable
          ...
          style={{
            backgroundColor:
              activeTab === "rules" ? (isDark ? "#1C1C1E" : "#F3F4F6") : "transparent",
          }}
        ...
        >
          ...
          <View
            ...
            style={{
              backgroundColor:
                activeTab === "rules" ? "rgba(129,140,248,0.1)" : isDark ? "#27272A" : "#E5E7EB",
            }}
          >
            <Text
              className="text-[10px]"
        ...
        <Pressable
          ...
          style={{
            backgroundColor:
              activeTab === "vocab" ? (isDark ? "#1C1C1E" : "#F3F4F6") : "transparent",
          }}
          onPress={() => setActiveTab("vocab")}
        >
          <Text
            className={`text-sm font-medium ${
              activeTab === "vocab" ? "text-foreground" : "text-muted-foreground"
            }`}
          >
            Vocabulary
          </Text>
          <View
            ...
            style={{
              backgroundColor:
                activeTab === "vocab" ? "rgba(129,140,248,0.1)" : isDark ? "#27272A" : "#E5E7EB",
            }}
          >
            <Text
              className="text-[10px]"
      ...
      {activeTab === "vocab" && <VocabularyTab vocabulary={content.vocabulary} isDark={isDark} />}
      ...
      {totalRules > 0 && understoodRules === totalRules && (
        ...
        >
          ...
          <Text className="text-lg font-semibold mt-2" style={{ color: "#10B981" }}>
            Great job!
          </Text>
