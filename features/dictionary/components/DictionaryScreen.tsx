...
export default function DictionaryScreen() {
  ...
  const { data: statsData, refetch: refetchStats } = useGetDictStatsQuery({
    langCode: "en",
  });

  useRefetchOnFocus([refetchWords, refetchStats]);
  const { refreshing, onRefresh } = useRefreshControl([refetchWords, refetchStats]);
      ...
      <View className="px-4 pb-3">
        <View className="flex-row items-center justify-between mb-2">
          ...
          <View className="flex-row items-center gap-3">
            ...
            <Pressable onPress={() => setAddingWord(!addingWord)} className="p-2 active:opacity-50">
        ...
        {addingWord && (
          <View className="flex-row gap-2 mb-3">
            ...
            >
              <Text className="text-white font-semibold text-sm">{t("dictionary.addBtn")}</Text>
            </Pressable>
          </View>
        )}
